...
do
  ...
  if [ $CONNTRACK -eq 19 ]; then
    awk 'BEGIN { printf "{ip_conntrack::"} { gsub(/(src|dst|sport|dport|mark|bytes)=/, ""); gsub(/\[[A-Z]+\]/, ""); printf "'\''%s'\'','\''%s'\'','\''%s'\'','\''%s'\'','\''%s'\'',%s,",$1,$1 == "tcp" ? $5 : $4,$1 == "tcp" ? $7 : $6,$1 == "tcp" ? $6 : $5,$1 == "tcp" ? $8 : $7,$(NF-1); } END { print "'\''-'\''}"}' /proc/net/ip_conntrack >> /tmp/traffic.dat
  else
    awk 'BEGIN { printf "{ip_conntrack::"} { gsub(/(src|dst|sport|dport|mark|bytes)=/, ""); gsub(/\[[A-Z]+\]/, ""); printf "'\''%s'\'','\''%s'\'','\''%s'\'','\''%s'\'','\''%s'\'',%s,",$1,$1 == "tcp" ? $5 : $4,$1 == "tcp" ? $7 : $6,$1 == "tcp" ? $6 : $5,$1 == "tcp" ? $8 : $7,$(NF-2); } END { print "'\''-'\''}"}' /proc/net/ip_conntrack >> /tmp/traffic.dat
  fi
